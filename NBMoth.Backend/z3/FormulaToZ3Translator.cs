...
public class FormulaToZ3Translator{
        ...
        private readonly LinkedList<BoolExpr> constraintList = new LinkedList<BoolExpr>();
        ...
        private readonly Z3TypeInference z3TypeInference;
        ...
        public List<Expr> getImplicitVariablesAsZ3Expression()
        {
            ...
            foreach (DeclarationNode decl in implicitDeclarations)
            {
    ...
    {
        return translateVariableEqualToExpr(name, exprNode, z3Context, z3TypeInference);
    }
                                                         ...
                                                         TranslationOptions opt, Z3TypeInference z3TypeInference)
    {
        ...
        Expr variable = z3Context.MkConst(name, z3Value.Sort);
    ...
    {
        FormulaToZ3Translator formulaToZ3Translator = new FormulaToZ3Translator(z3Context, z3TypeInference);
        FormulaToZ3TranslatorVisitor visitor = new FormulaToZ3TranslatorVisitor();
    ...
    {
        switch (constraintList.Count)
        {
            case 0:
                return null;
            case 1:
                return constraintList.First();
            default:
    ...
    //public class FormulaToZ3TranslatorVisitor : FormulaVisitor<Expr, TranslationOptions>
    //{

    //    public class OperatorNotImplementedError : AssertionError
    //    {
    ...
    //    public override Expr visitIdentifierExprNode(IdentifierExprNode node, TranslationOptions ops)
    //    {
    //        return z3Context.mkConst(node.getName(), getZ3Sort(node.getDeclarationNode()));
    //    }

    //    public override Expr visitCastPredicateExpressionNode(CastPredicateExpressionNode node, TranslationOptions expected)
    //    {
    //        return visitPredicateNode(node.getPredicate(), expected);
    //    }


    //    public override Expr visitIdentifierPredicateNode(IdentifierPredicateNode node, TranslationOptions ops)
    //    {
    //        return z3Context.mkBoolConst(node.getName());
    //    }


    //    public override Expr visitPredicateOperatorWithExprArgs(PredicateOperatorWithExprArgsNode node, TranslationOptions ops)
    //    {
    //        readonly List<Expr> arguments = node.getExpressionNodes().stream().map(it->visitExprNode(it, ops))
