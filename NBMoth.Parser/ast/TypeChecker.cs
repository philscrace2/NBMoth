...
namespace NBMoth.Parser.ast {
    public class TypeChecker : AbstractVisitor<BType, BType>
    {
        ...
        private static readonly string TYPE_ERROR = "TYPE_ERROR";
    ...
    public static void typecheckLTLFormulaNode(LTLFormula ltlFormulaAst)
    {
        TypeChecker typeChecker = new TypeChecker();
    try
    {
...
machineNode.getDeferredSets().stream().filter(ds->ds.getType() == null).forEach(ds->
    ds.setType(new SetType(new DeferredSetElementType(ds.getName()))));
